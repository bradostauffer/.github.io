...
unsigned int IndexMinPQ<K>::Top(void) {
  ...
  // TODO(bwstauff): return index at top of priority queue
}
...
void IndexMinPQ<K>::Push(const K &key, unsigned int idx) {
  ...
  PercolateDown(1);
  // TODO(bwstauff): push key-value pair made of @key and @idx
...
void IndexMinPQ<K>::Pop() {
  ...
  idx_to_heap[heap_to_idx[1]] = 0;
  heap_to_idx[1] = heap_to_idx[cur_size];
  heap_to_idx.pop_back();
  PercolateDown(1);
  cur_size--;
  // TODO(bwstauff): remove min item
...
void IndexMinPQ<K>::ChangeKey(const K &key, unsigned int idx) {
  ...
  PercolateDown(1);
  PercolateUp(cur_size);
  // TODO(bwstauff): modify the key associated to index @idx
