...
void LLRB_map<K, V>::Insert(std::unique_ptr<Node> &n,
  ...
  else if (key < n->key && key != n->key)
    Insert(n->left, key, value);
  else if (key > n->key && key != n->key)
    Insert(n->right, key, value);
  else if (key == n->key)
    throw std::runtime_error("Key already has one value");
  else
...
template <typename K, typename V>
 typename LLRB_map<K, V>::Node* LLRB_map<K, V>::Get(Node* n, const K& key) {
  while (n) {
     if (key < n->key) {
        n = n->left.get();
        continue;
